...
//-----------------------------------------------------------------------------
//added gonna add from bship/snake
#define MAXBUTTONS 4
...
int nbuttons=0;
//added
extern void MainMenu(int x,int y, GLuint textid);
extern void Credit(GLuint textid);
class Image;
...
class Global {
	...
	GLuint groundTile;
	GLuint l1BG;
	GLuint titlescreenTexture;// compared to mainMenuTexture added
	int xres, yres;
	char keys[65536];
	double camera[2];
	int showCredit;
	...
	Global() {
	    ...
	    showCredit = 0;
	    player = 0;//added
	    currentLevel = 0;//added changed to 0
	    //need to add nbuttons
	}
} gl;
...
//added by Joshua C
//added bools to tell when its in menu & the given actions to take inside menu
bool mainmenu = true;
bool tutorialmenu = false; //as it is still in the main menu w/out having taken the tutorial screen
bool creditsmenu = false; // if I change somethings to make it more organized
bool startgame = false; // to start the game
bool lvl1=false;//added to accomedate the lvl
//gonna need to use an int system to keep track of where the 
int posarray = 1; //position of arrow
...
void init_opengl(void)
{
    ...
    //texture for Level 1 Tiles
    glGenTextures(1, &gl.initialTile);
    glBindTexture(GL_TEXTURE_2D, gl.initialTile);
    ...
    glGenTextures(1, &gl.middleTile);
    glBindTexture(GL_TEXTURE_2D, gl.middleTile);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img[10].width, img[10].height, 0, GL_RGB, GL_UNSIGNED_BYTE, img[10].data);
    glActiveTexture(GL_TEXTURE0);

    glGenTextures(1, &gl.endTile);
    glBindTexture(GL_TEXTURE_2D, gl.endTile);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    ...
    glGenTextures(1, &gl.groundTile);
    glBindTexture(GL_TEXTURE_2D, gl.groundTile);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    glTexImage2D(GL_TEXTURE_2D, 0, 3, img[14].width, img[14].height, 0, GL_RGB, GL_UNSIGNED_BYTE, img[14].data);
    glActiveTexture(GL_TEXTURE0);

    glGenTextures(1, &gl.l1BG);
    glBindTexture(GL_TEXTURE_2D, gl.l1BG);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    glTexImage2D(GL_TEXTURE_2D, 0, 3, img[15].width, img[15].height, 0, GL_RGB, GL_UNSIGNED_BYTE, img[15].data);
    glActiveTexture(GL_TEXTURE0);
    ...
    //added
    //Texture for titlescreen Joshua C
    glGenTextures(1, &gl.titlescreenTexture);
    glBindTexture(GL_TEXTURE_2D, gl.titlescreenTexture);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, 3, img[16].width, img[16].height, 0,GL_RGB, GL_UNSIGNED_BYTE, img[16].data);
...
int check_keys(XEvent *e)
{
    ...
    //new changes starting here added
    if(mainmenu) { //main menu true
	if(key=XK_Escape) {
	    exit(1);
	}
	if(key== XK_Return) { //XK_Return what does that mean
	    if (posarray == 1) {
		startgame = true;
		mainmenu = false;
		//for the moment will need to comment out
	...
	if(posarray > 4) {// from bottom menu to the top
	    posarray = 1;
	}
	if(posarray < 1) {//means to be able to from top to bottom of menu
	    posarray = 4;
	}
    }
    if (tutorialmenu) {
...
void render()
{
    ...
    //added gonna make an external function which takes care of the main menu
    //for the moment instead of making it an external function gonna put it in here just to see
    if(mainmenu) {
	//modified from example
	MainMenu(gl.xres,gl.yres,gl.titlescreenTexture);
    }
    /*
    //added
    //Clear the screen
    ...
    //added from prev try to check
    //add text into the background use example from snake/bship added
    //initialize buttons...
    nbuttons=0;
    //size and position
    button[nbuttons].r.width = 140;
    button[nbuttons].r.height = 60;
    button[nbuttons].r.left = 850;
...
extern void drawTile(int currentLevel, int width, int lenfth, GLuint id);
extern Level getLevel(int currentLevel);
extern void drawBackground(int width, int length, GLuint id);
if (creditsmenu) {//changed to creditsmenu og gl.showCredit
    Credit(gl.xres,gl.yres,gl.creditTexture);
...
/*
   ...
   */
// }
if (gl.currentLevel == 1) {
    drawBackground(gl.xres, gl.yres, gl.l1BG);
}
/*
   Level l = getLevel(gl.currentLevel);
   int width;
   int height;
   int renderingCols = gl.xres / l.tilesize[0] + 2;
   int colIdx = static_cast<int>((int) gl.camera[0] / l.dbsz[0]);
   for (int cols = 0; cols < renderingCols; cols++) {
   int rowIdx = l.nrows -1;
   for (int rows = 0; rows < l.nrows; rows++) {
   width = static_cast<int>(cols * l.dbsz[0]);
   height = static_cast<int>(rows * l.dbsz[1]);
//printf("%c\n", l.arr[rowIdx][colIdx]);
printf("row %d col %d is : %c\n", rowIdx, colIdx, l.arr[rowIdx][colIdx]);
if (l.arr[rowIdx][colIdx] == 'i') {
drawTile(gl.currentLevel, width, height, gl.initialTile);
}

if (l.arr[rowIdx][colIdx] == 'm') {
drawTile(gl.currentLevel, width, height, gl.middleTile);
}

if (l.arr[rowIdx][colIdx] == 'f') {
drawTile(gl.currentLevel, width, height, gl.endTile);
}

if (l.arr[rowIdx][colIdx] == 'w') {
drawTile(gl.currentLevel, width, height, gl.waterTile);
}

if (l.arr[rowIdx][colIdx] == 'g') {
drawTile(gl.currentLevel, width, height, gl.groundTile);
}
--rowIdx;
}
colIdx = (colIdx+1) % l.ncols;
}
if (gl.player) {
glBindTexture(GL_TEXTURE_2D, gl.playerTexture);
glColor3ub(0,0,0);
...
glEnd();
playerSprite(150, 150, gl.playerTexture);
}*/  
}
